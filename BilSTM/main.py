...
def apply_clustering(feature_vectors):
    """
    Apply KMeans clustering on the given feature vectors.

    Args:
    - feature_vectors (list): A list of tuples containing feature vectors and corresponding labels.

    Returns:
    - labels (numpy.ndarray): Cluster labels assigned by KMeans.
...
# L2 and Dropout
# def train_bilstm_model(X_train, y_train, l2_penalty=0.001, dropout_rate=0.2):
#     """
#     Train a BiLSTM model with both L2 regularization and dropout regularization and plot loss and accuracy.
...
#     """
#     model = Sequential()
#     model.add(Bidirectional(LSTM(32, input_shape=(X_train.shape[1], X_train.shape[2]), kernel_regularizer=l2(l2_penalty))))
#     model.add(Dropout(dropout_rate))  # Adding dropout after the BiLSTM layer
...
#0.3 dropout and L2
# def train_bilstm_model(X_train, y_train, l2_penalty=0.001, dropout_rate=0.3, lstm_units=64, learning_rate=0.0001):
#     """
#     Train a BiLSTM model with improved hyperparameters and plot loss and accuracy.
...
#     """
#     model = Sequential()
#     model.add(Bidirectional(LSTM(lstm_units, input_shape=(X_train.shape[1], X_train.shape[2]), kernel_regularizer=l2(l2_penalty))))
#     model.add(Dropout(dropout_rate))  # Adding dropout after the BiLSTM layer
#     model.add(Dense(128, activation='relu', kernel_regularizer=l2(l2_penalty)))
#     model.add(Dropout(dropout_rate))  # Adding dropout after the dense layer
#     model.add(Dense(3, activation='softmax', kernel_regularizer=l2(l2_penalty)))
...
#new try
def train_bilstm_model(X_train, y_train, l2_penalty=0.002, dropout_rate=0.15, lstm_units=64, learning_rate=0.0001, epochs=80):
    """
    Train a BiLSTM model with further adjustments to hyperparameters and plot loss and accuracy.
...
#Dropout 0.2
# def train_bilstm_model(X_train, y_train, dropout_rate=0.2):
#     """
#     Train a BiLSTM model with dropout regularization and plot loss and accuracy.
...
# #L2 Regularization 
# def train_bilstm_model(X_train, y_train, l2_penalty=0.001):
#     """
#     Train a BiLSTM model with L2 regularization and plot loss and accuracy.
...
#     """
#     model = Sequential()
#     model.add(Bidirectional(LSTM(32, kernel_regularizer=l2(l2_penalty)), input_shape=(X_train.shape[1], X_train.shape[2])))
#     model.add(Dense(128, activation='relu', kernel_regularizer=l2(l2_penalty)))
#     model.add(Dense(3, activation='softmax', kernel_regularizer=l2(l2_penalty)))
#     model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
#     history = model.fit(X_train, y_train, epochs=60, batch_size=32, validation_split=0.2)
...
def main(training_dir, feature_type='mfcc'):
    ...
    # Training the Model
    model, history = train_bilstm_model(X_train, y_train)
